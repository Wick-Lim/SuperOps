...
// The garbage-collector tests run against a real Postgres, on purpose.
//
// What they assert is not expressible against a mocked pool: that ListOrphans
// and StorageKeysPresent — the two predicates that decide whether a user's file
// is garbage — name every column and every table that can own an object. A
// hand-rolled fake would be a second implementation of the SQL, and it would
// agree with whatever the SQL happened to say.
//
...
var testDBName = fmt.Sprintf("superops_file_test_%d", os.Getpid())
...
func testDB(t *testing.T) *pgxpool.Pool {
	...
	if dbErr != nil {
		...
		t.Skipf("postgres unavailable, skipping file database tests: %v", dbErr)
	}
	return dbPool
}
