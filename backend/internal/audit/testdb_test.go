...
// The partition, chain and coalescing tests run against a real Postgres, on
// purpose. None of them is expressible against a mocked pool: what is under test
// is a monthly RANGE partition rotating, a retention DROP freeing a whole
// partition, an ON CONFLICT arbiter that has to include the partition key, and a
// hash chain surviving an UPDATE issued behind the application's back.
...
var testDBName = fmt.Sprintf("superops_audit_test_%d", os.Getpid())
...
func testDB(t *testing.T) *pgxpool.Pool {
	...
	if dbErr != nil {
		...
		t.Skipf("postgres unavailable, skipping audit database tests: %v", dbErr)
	}
	return dbPool
}
