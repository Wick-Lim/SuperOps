...
// The authz tests run against a real Postgres, on purpose.
//
// Every method in this package is a SQL predicate; a mocked pgx would assert
// that the Go around the query is unchanged while leaving the query — the part
// that actually decides whether one tenant can read another's channels —
// completely unchecked. So these tests create a throwaway database, apply the
// real migrations to it, insert real rows and assert on the real answers.
//
...
// race.
var testDBName = fmt.Sprintf("superops_authz_test_%d", os.Getpid())
