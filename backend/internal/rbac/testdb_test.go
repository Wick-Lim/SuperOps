...
// These middleware tests run against a real Postgres, on purpose.
//
// Both gates delegate their decision to authz.Checker, which is a set of SQL
// predicates. A fake checker would only prove that the middleware calls
// something; running the real one against real rows proves the gate admits and
// refuses the callers it is documented to. So these tests create a throwaway
// database, apply the real migrations to it, insert real rows and assert on the
// real answers.
//
...
// race.
var testDBName = fmt.Sprintf("superops_rbac_test_%d", os.Getpid())
