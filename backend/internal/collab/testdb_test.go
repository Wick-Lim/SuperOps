...
// These tests run against a real Postgres, on purpose.
//
// The two properties this package exists to guarantee — a gap-free log, and a
// compaction that cannot delete an update a concurrent writer has not committed
// yet — are properties of Postgres row locking and of the order the statements
// are issued in. A mocked pgx would assert that the Go around the queries is
// unchanged while leaving the only interesting part untested.
//
// Reachability is decided from the standard DB_* environment, matching
// internal/authz. Without Postgres the suite skips, unless
// SUPEROPS_REQUIRE_INFRA=1 forces it to fail.

var testDBName = fmt.Sprintf("superops_collab_test_%d", os.Getpid())
...
func testDB(t *testing.T) *pgxpool.Pool {
	...
	if dbErr != nil {
		...
		t.Skipf("postgres unavailable, skipping collaboration database tests: %v", dbErr)
	}
	return dbPool
}
