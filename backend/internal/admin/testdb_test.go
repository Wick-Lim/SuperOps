...
// The admin handler tests run against a real Postgres, on purpose.
//
// Every endpoint in this package is a scoping predicate expressed in SQL —
// `workspace_id = ANY($1)`, a guard on an UPDATE, a session delete inside the
// deactivation transaction. A mocked pool would assert the shape of the Go and
// leave every one of those unchecked, which is precisely where the cross-tenant
// bugs were. So these tests create a throwaway database, apply the real
// migrations to it, insert real rows and assert on the real answers.
//
...
// (the same switch test/integration uses). Note that CI's unit job has no
// Postgres service, so these currently skip there — see the note in the task
// summary.
...
// race.
var testDBName = fmt.Sprintf("superops_admin_test_%d", os.Getpid())
...
func testDB(t *testing.T) *pgxpool.Pool {
	...
	if dbErr != nil {
		...
		t.Skipf("postgres unavailable, skipping authz database tests: %v", dbErr)
	}
	return dbPool
}
