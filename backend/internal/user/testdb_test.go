...
// The user tests run against a real Postgres, on purpose.
//
// The interesting behaviour of this package lives in SQL: the column list that
// decides whether a status write is ever readable again, the throttle predicate
// on TouchLastActive, and above all the shared-workspace / block filtering in
// SearchInSharedWorkspaces. None of that is observable through a mocked pool,
// so these tests create a throwaway database, apply the real migrations to it,
// insert real rows and assert on the real answers.
//
...
// race.
var testDBName = fmt.Sprintf("superops_user_test_%d", os.Getpid())
