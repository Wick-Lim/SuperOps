...
// These tests run against a real Postgres, and they have to.
//
// The property under test is what TWO CONCURRENT TRANSACTIONS do to one row.
// That is not expressible against a fake: a mock would answer whatever the mock
// author believed about ON CONFLICT ... DO UPDATE ... WHERE, and the belief is
...
var testDBName = fmt.Sprintf("superops_quota_test_%d", os.Getpid())
...
func testDB(t *testing.T) *pgxpool.Pool {
	...
	if dbErr != nil {
		...
		t.Skipf("postgres unavailable, skipping quota database tests: %v", dbErr)
	}
	return dbPool
}
